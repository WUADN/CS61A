...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        cur, count = self.place, 0
        while count < self.lower_bound:
            if cur.is_hive:
                return None
            cur = cur.entrance
            count += 1
        while count <= self.upper_bound:
            if cur.is_hive:
                return None
            elif cur.bees:
...
class LaserAnt(ThrowerAnt):
    ...
    def insects_in_front(self):
        ...
        if self.place.ant.is_container:
            res[self.place.ant] = 0
        if self.place.bees:
            for bee in self.place.bees:
                res[bee] = 0
        cur, far = self.place.entrance, 1
        while not cur.is_hive:
            ...
            if cur.ant:
                res[cur.ant] = far
                if cur.ant.is_container:
                    res[cur.ant.contained] = far
            far += 1
            cur = cur.entrance
        return res
        # END Problem Optional 2
    ...
    def calculate_damage(self, distance):
        # BEGIN Problem Optional 2
        res = self.damage - self.insects_shot * 0.0625 - distance * 0.25
        if res >= 0:
            return res
        return 0
        # END Problem Optional 2
