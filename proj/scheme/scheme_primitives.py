    ...
    def print_repr(self):
        ...
        if s[0] == "'":
            ...
            s = SchemeStr._escape.sub(
                lambda x: (r"\"" if x == '"' else "'" if x == r"\'" else x), s
            )
            s = '"' + s + '"'
        return s
...
class Pair(SchemeValue):
    ...
    def __init__(self, first, second):
        ...
        assert isinstance(first, SchemeValue) and isinstance(
            second, SchemeValue
    ...
    def __repr__(self):
        ...
        return "Pair({0!r}, {1!r})".format(uncoerce(self.first), uncoerce(self.second))
